...
<style>
  ...
  .surface{ background:#fff; border:1px solid #e5e7eb; border-radius:1rem; box-shadow:0 1px 1px rgba(0,0,0,.02),0 2px 6px rgba(0,0,0,.04); }
  .surface:hover{ box-shadow:0 2px 4px rgba(0,0,0,.04),0 8px 24px rgba(0,0,0,.08); }
  .soft-pill{ display:inline-flex; align-items:center; gap:.5rem; padding:.375rem .65rem; font-size:.72rem; font-weight:600; border-radius:999px; border:1px solid #e5e7eb; background:#fafafa; color:#334155; }
  .ring-gradient{ position:relative; }
  .ring-gradient::before{ content:''; position:absolute; inset:-1px; z-index:-1; border-radius:1rem; background:linear-gradient(120deg, rgba(220,38,38,.35), rgba(22,163,74,.35), rgba(37,99,235,.35)); opacity:0; transition:opacity .25s ease; }
  .ring-gradient:hover::before{ opacity:1; filter:saturate(1.2); }
  .icon-chip{ border-radius:.8rem; display:inline-grid; place-items:center; width:40px; height:40px; background:#f8fafc; border:1px solid #e2e8f0; }
  ...
  .btn{ border-radius:.65rem; font-weight:700; font-size:.85rem; padding:.55rem .9rem; transition:transform .12s ease, filter .15s ease; }
  ...
  .ribbon{ background:
      ...
      linear-gradient(90deg,#ffffff,#fafafa); border:1px solid #e5e7eb; border-radius:1rem; }
...
<div class="p-6 text-gray-900 space-y-6">
  ...
  <div class="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
    ...
    {{-- Account --}}
    <a href="{{ route('settings.account') }}" class="surface ring-gradient p-5 rounded-2xl transition fade-in group block cursor-pointer">
    ...
    {{-- Appearance (now clickable) --}}
    <a href="{{ route('settings.appearance') }}" class="surface ring-gradient p-5 rounded-2xl transition fade-in md:col-span-2 xl:col-span-1 group block cursor-pointer">
      <div class="flex items-start gap-4">
        <div class="icon-chip">🎨</div>
        <div class="flex-1">
          <div class="flex items-center gap-2">
    ...
    {{-- Billing card removed as requested --}}
