...
<style>
  :root{
    ...
    --card:#ffffff; --shadow:0 8px 28px rgba(15,23,42,.08);
    --emerald:#10b981; --blue:#2563eb; --red:#ef4444; --amber:#f59e0b; --indigo:#6366f1;
    --bg-card: #fff;
  }
  body{ background:var(--bg); color:var(--ink); }

  /* Buttons (local light theme) */
  .btn{ display:inline-flex; align-items:center; gap:.5rem; padding:.55rem .85rem;
        border-radius:.75rem; font-weight:600; border:1px solid transparent; transition:.18s ease;
        box-shadow: 0 1px 2px rgba(0,0,0,.04); }
  .btn svg{ width:1rem;height:1rem }
  .btn:disabled{ opacity:.45; cursor:not-allowed }
  .btn-primary{ background:var(--red); color:#fff; }
  .btn-primary:hover{ filter:brightness(.95) }
  .btn-secondary-green{ background:#ecfdf5; color:#065f46; border-color:#a7f3d0; }
  .btn-secondary-green:hover{ background:#d1fae5 }
  .btn-secondary-blue{ background:#eff6ff; color:#1e40af; border-color:#bfdbfe; }
  ...
  .pill{
    ...
    border-radius:9999px; font-size:.72rem; font-weight:700; letter-spacing:.02em;
    border:1px solid;
  }
  ...
  .pill-amber{  background:#fffbeb; color:#78350f; border-color:#fde68a; }
  ...
  .soft-card{
    background:var(--card); border:1px solid var(--line); border-radius:1rem; box-shadow:var(--shadow);
  }
  .kbd{
    background:#f3f4f6; border:1px solid #e5e7eb; border-bottom-width:2px; border-radius:.5rem;
    padding:.1rem .4rem; font-size:.75rem;
  }
  .tbl{ width:100%; border-collapse:separate; border-spacing:0; }
  .tbl thead th{
    font-weight:800; font-size:.78rem; letter-spacing:.02em; text-transform:uppercase; color:#334155;
    background:#f8fafc; position:sticky; top:0; z-index:10; border-bottom:1px solid var(--line);
  }
  .tbl th,.tbl td{ padding:.9rem 1rem; }
  .tbl tbody tr{ transition: background .12s ease, transform .12s ease; }
  .tbl tbody tr:hover{ background:#f9fafb; }
  .tbl tbody tr + tr td{ border-top:1px solid var(--line); }

  /* Tiny utility */
  .chip{
    display:inline-flex; align-items:center; gap:.35rem; padding:.25rem .5rem; font-size:.72rem; font-weight:700;
    border-radius:.5rem; background:#eef2ff; color:#3730a3; border:1px solid #c7d2fe;
  }

  /* Toast */
  .toast{ position:fixed; right:1rem; bottom:1rem; z-index:60; background:#111827; color:#fff;
          padding:.75rem 1rem; border-radius:.75rem; box-shadow:0 10px 30px rgba(0,0,0,.18);
          opacity:0; transform:translateY(10px); transition:.2s ease; }
  .toast.show{ opacity:1; transform:translateY(0); }

  /* Subtle icon box */
  .iconbox{
    height:42px;width:42px;border-radius:12px;background:#fff;border:1px solid var(--line);
    display:grid;place-items:center; box-shadow:0 2px 8px rgba(0,0,0,.04);
  }
</style>
@endsection
    ...
    <div class="px-6 py-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      <div class="flex items-center gap-3">
        <div class="iconbox">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-red-500" viewBox="0 0 24 24" fill="currentColor">
            <path d="M9 3a1 1 0 0 0-1 1v1H5.5a1 1 0 0 0-.96.72l-1.5 5A1 1 0 0 0 4 12h.5v7a2 2 0 0 0 2 2h11a2 2 0 0 0 2-2v-7H20a1 1 0 0 0 .96-1.28l-1.5-5a1 1 0 0 0-.96-.72H16V4a1 1 0 0 0-1-1H9Zm2 10a1 1 0 1 1 2 0v4a1 1 0 1 1-2 0v-4Zm-3 0a1 1 0 1 1 2 0v4a1 1 0 1 1-2 0v-4Zm8 0a1 1 0 1 1 2 0v4a1 1 0 1 1-2 0v-4ZM10 5h4v1h-4V5Z"/>
          </svg>
        </div>
        <div>
      ...
      <div class="flex items-center gap-2">
        <a href="{{ route('production.index') }}" class="btn btn-secondary-blue">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-width="2" d="m10 19-7-7 7-7m-7 7h18"/></svg>
          Back
        </a>
          ...
          <div class="relative">
            ...
            <span class="absolute right-2 top-2.5 text-gray-400">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                <circle cx="11" cy="11" r="7" stroke-width="2"/><path d="m20 20-3.5-3.5" stroke-width="2"/>
              </svg>
            </span>
          </div>
  ...
  <div class="soft-card p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
    ...
    <div class="flex items-center gap-2">
      <button id="bulkRestore" type="button" class="btn btn-secondary-green" disabled>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="currentColor" d="M13 3a9 9 0 1 0 8.94 8H20a7 7 0 1 1-2.05-4.95L16 8h5V3l-1.64 1.64A8.96 8.96 0 0 0 13 3Z"/></svg>
        Bulk Restore
      </button>
      <button id="bulkDelete" type="button" class="btn btn-primary" disabled>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="currentColor" d="M9 3h6a1 1 0 0 1 1 1v1h4v2H4V5h4V4a1 1 0 0 1 1-1Zm1 6h2v8h-2V9Zm4 0h2v8h-2V9Zm-8 0h2v8H6V9Z"/></svg>
        Bulk Delete Forever
      </button>
    </div>
  </div>
      ...
      <tbody>
        @forelse($items as $p)
          <tr>
            ...
            <td class="px-4 py-3 align-top">
              <div class="font-medium">{{ $p->product?->product_name ?? '—' }}</div>
              @if($p->parentProduct && $p->parent_product_id && $p->parent_product_id !== $p->product_id)
                <div class="text-xs text-gray-500">Parent: {{ $p->parentProduct->product_name }}</div>
              @endif
            </td>
            <td class="px-4 py-3 align-top">
              <span class="chip">
            ...
            <td class="px-4 py-3 align-top">
              <div class="flex items-center gap-2 justify-end">
                <button type="button" class="btn btn-secondary-green px-3 py-1" data-action="restore" data-id="{{ $p->id }}">
                  Restore
                </button>
