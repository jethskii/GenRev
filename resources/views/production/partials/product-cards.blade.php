...
<style>
  ...
  .stat-label{ color:#6b7280; font-size:.8rem; letter-spacing:.01em; text-transform:uppercase; }
  .stat-val{ color:#0f172a; font-weight:800; font-size:.98rem; }
  ...
  .neo-btn{
    ...
    border-radius:999px; font-weight:700; font-size:.92rem; line-height:1;
    background:radial-gradient(circle at top,#ffffff,#f3f4ff);
    color:#111827; border:1px solid #e6e8f0;
    box-shadow:
...
@forelse($products as $p)
  ...
  <div id="product-card-{{ $p->id }}"
    ...
    <div class="prod-card-media">
      ...
      @if(!empty($p->category))
        <span class="absolute top-2 right-2 px-2.5 py-0.5 rounded-full text-[0.65rem] uppercase tracking-wide bg-white/90 border border-indigo-100 text-indigo-700 shadow-sm">
          {{ $p->category }}
        </span>
      @endif
    </div>
