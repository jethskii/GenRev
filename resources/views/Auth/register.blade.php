...
<style>
  ...
  .auth-center{ min-height:100vh; display:flex; align-items:center; justify-content:center; padding:2.5rem 1rem; }
  ...
  .underline::after{ content:''; position:absolute; left:0; right:0; bottom:-4px; height:4px; border-radius:999px; background:linear-gradient(90deg,var(--red),var(--green) 60%,var(--blue)); opacity:.25; }
  ...
  .btn{
    display:inline-flex; justify-content:center; align-items:center; gap:.5rem; width:100%;
      ...
      <form method="POST" action="{{ route('register.submit') }}" id="regForm" novalidate>
        ...
        <div class="row">
          ...
          <input
            ...
            aria-describedby="@error('position') positionHelp @enderror">
          @error('position') <p id="positionHelp" class="help" style="color:#b91c1c">{{ $message }}</p> @enderror
        </div>
