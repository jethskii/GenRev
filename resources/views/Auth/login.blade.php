...
<style>
  ...
  /* Page + components (same as your original) */
  .liquid-bg{position:relative;min-height:100vh;background:linear-gradient(135deg,#1F1E1E 0%,#001C00 100%);font-family:'Inria Sans',sans-serif;overflow:hidden}
  .liquid-bg::before{content:'';position:absolute;top:-50%;left:-50%;width:200%;height:200%;background:linear-gradient(to bottom right,rgba(18,108,7,.15) 0%,rgba(113,200,98,.15) 25%,rgba(210,220,50,.12) 50%,rgba(113,200,98,.15) 75%,rgba(10,56,14,.15) 100%);transform:rotate(30deg);animation:liquidFlow 15s linear infinite;z-index:0;opacity:.5}
  ...
  .liquid-card{position:relative;overflow:hidden;border-radius:20px;backdrop-filter:blur(10px);background:rgba(31,30,30,.7);box-shadow:0 8px 32px rgba(0,28,0,.3);transition:all .5s ease;z-index:1}
  .liquid-card::before{content:'';position:absolute;inset:0;background:linear-gradient(45deg,rgba(4,119,5,.10) 0%,rgba(237,209,0,.10) 50%,rgba(4,119,5,.10) 100%);animation:cardShine 8s ease infinite;z-index:-1}
  @keyframes cardShine{0%{opacity:.3}50%{opacity:.1}100%{opacity:.3}}
  .liquid-input{background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.15);color:#fff;border-radius:10px;padding:10px 14px;width:100%;transition:all .3s ease}
  ...
  .liquid-btn{position:relative;overflow:hidden;border:none;border-radius:12px;background:linear-gradient(90deg,#047705 0%,#0aad0a 100%);color:#fff;font-weight:600;box-shadow:0 4px 15px rgba(4,119,5,.4);transition:transform .2s ease,box-shadow .2s ease}
  .liquid-btn::before{content:'';position:absolute;inset:0;left:-100%;background:linear-gradient(90deg,transparent,rgba(255,255,255,.25),transparent);transition:left .6s ease}
  .liquid-btn:hover{transform:translateY(-1px);box-shadow:0 6px 18px rgba(4,119,5,.45)}
