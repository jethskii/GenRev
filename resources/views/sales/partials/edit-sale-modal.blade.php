...
<div
  id="editSaleModal"
  class="modal-wrap hidden"
  role="dialog"
  aria-modal="true"
  aria-labelledby="editSaleTitle"
>
  <div
        ...
        <div class="grid grid-cols-1 gap-y-5 sm:grid-cols-6">
          ...
          <div class="sm:col-span-3">
            ...
            <select id="edit-production-id" name="production_id" class="select">
              ...
              @if(isset($batches) && count($batches))
...
<script>
(function () {
  const API_READ_URL = (id) =>
    (window.saleApiUrl ? `${window.saleApiUrl}/${id}` : `{{ url('/api/sales') }}/${id}`);
  ...
  const fmtDate = (v) => {
    ...
    // Normalize to local yyyy-mm-dd, adjusting for TZ offset
  ...
  function wireFieldEvents() {
    ...
    if (prodSel && !prodSel._wired) {
      prodSel.addEventListener('change', () => {
        ...
        if ((price.value === '' || +price.value === 0) && p !== null) {
          price.value = p;
        }
      });
      prodSel._wired = true;
  ...
  async function fetchSaleJson(id) {
    try {
      const res = await fetch(API_READ_URL(id), {
  ...
  async function openEditModal(saleOrId) {
    ...
    if (typeof saleOrId === 'number' || /^\d+$/.test(String(saleOrId))) {
      ...
      if (!sale) return;
    } else if (typeof saleOrId === 'object' && saleOrId !== null) {
      sale = saleOrId;
    } else {
      console.warn('openEditModal called with invalid param', saleOrId);
      return;
    }
    ...
    if (!idInput || !prodSel || !dateInput || !qtyInput || !unitSel || !priceInput || !statusSel || !form) {
      console.error('Edit sale modal fields not found');
      return;
    }
    ...
    // Product select; if not found, leave first option
    const productId = sale.product_id ?? '';
    if (productId && prodSel.querySelector(`option[value="${productId}"]`)) {
      prodSel.value = String(productId);
    } else if (prodSel.options.length) {
    ...
    priceInput.value = (unitPrice !== null ? unitPrice : '');
    orderNoEl.value  = sale.order_number || sale.invoice_number || '';

    const allowedStatuses = ['Paid', 'Pending', 'Completed', 'Cancelled'];
    statusSel.value = allowedStatuses.includes(status) ? status : 'Completed';

    // Setup batch (if list is present in DOM)
    if (batchSel) {
      const productionId = sale.production_id ?? '';
      if (productionId && batchSel.querySelector(`option[value="${productionId}"]`)) {
        batchSel.value = String(productionId);
      } else {
        batchSel.value = '';
      }
      const opt = batchSel.options[batchSel.selectedIndex];
