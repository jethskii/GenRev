...
<head>
  ...
  <!-- Fonts (to match Cooperatiba vibe) -->
  <link href="https://fonts.googleapis.com/css2?family=Kalam:wght@400;700&display=swap" rel="stylesheet">
  <link href="https://fonts.googleapis.com/css2?family=Inria+Sans:wght@300;400;700&display=swap" rel="stylesheet">

  <!-- Modern Liquid/Glass UI Theme (CLONED) -->
  <style>
    :root {
      /* Dark liquid palette + neon accents */
      --navbar: linear-gradient(135deg, #1F1E1E 0%, #100E00 80%);
      --sidebar: rgba(31, 30, 30, 0.55);
      --sidebar-hover: rgba(4, 119, 5, 0.24);
      --sidebar-active: #EDD100; /* Cooperatiba yellow */
    ...
    .bg-navbar:hover {
      box-shadow: 0 8px 28px rgba(0, 123, 0, 0.35), 0 0 0 1px rgba(4,119,5,.2) inset;
    }

    /* Brand heading flair */
    .brand-title { font-family: 'Kalam', cursive; letter-spacing: .02em; text-shadow: -2px 1px 0px var(--brand-green); }

    /* Sidebar */
    aside { border-right: .5px solid var(--dark-line); }
    aside .p-6.text-2xl { font-family: 'Kalam', cursive; color: #fff; text-shadow: -2px 1px 0px var(--brand-green); }
    ...
    /* Active pill */
    .bg-sidebar-active {
      background: var(--sidebar-active) !important; color: #1F1E1E !important; font-weight: 700; position: relative;
      box-shadow: 0 6px 18px rgba(237,209,0,.25);
    }
    ...
    /* Underline shimmer + ripple clone for nav links */
    ...
    @media (max-width: 1024px) {
      aside { position: fixed; z-index: 50; transform: translateX(-100%); transition: transform 0.3s ease; }
      aside.open { transform: translateX(0); }
      body { padding-left: 0; }
    }

    /* Shine utility for cards/sections */
        ...
        <div class="h-full grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <div class="glass section-liquid-shine border border-dark-line shadow-md p-5 rounded-2xl backdrop-blur-lg">
            <div class="flex items-center justify-between mb-2">
              ...
              <span class="text-xs text-[var(--muted)]">Total: {{ number_format($totalMaterialsWeight, 2) }} kg</span>
            </div>
            @php
              $recentMaterials = $recentMaterials ?? collect();
            @endphp
...
<div class="glass section-liquid-shine border border-dark-line shadow-md p-5 rounded-2xl backdrop-blur-lg">
  ...
  <script>
    document.addEventListener("DOMContentLoaded", () => {
      ...
      // Sales Chart
      showIfEmpty(qty.concat(rev), 'salesEmpty');
        ...
        new Chart(salesCtx, {
          ...
          options: {
            ...
            plugins: {
              ...
              tooltip: { callbacks: { label: (ctx) => ctx.dataset.type === 'line' ? `Revenue: ₱${Number(ctx.parsed.y).toLocaleString(undefined,{ minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : `Qty: ${ctx.parsed.y}` } }
            },
            scales: {
