...
<style>
  ...
  .btn{border-radius:12px;padding:.55rem .9rem;border:1px solid var(--border);background:#fff}
  .btn-primary{background:linear-gradient(180deg,#f5892d,var(--tangerine));color:#fff;border:1px solid rgba(239,119,34,.45)}
  .btn:hover{filter:brightness(1.03)}
  .input{height:40px;border-radius:12px;border:1px solid var(--border);padding:.55rem .75rem;background:#fff}

  /* KPIs */
  .kpi .icon{width:40px;height:40px;border-radius:12px;border:1px solid var(--border);display:grid;place-items:center}
  ...
  .table thead th{position:sticky;top:0;background:#f9fafb;border-bottom:1px solid var(--border);font-size:.72rem;letter-spacing:.06em;text-transform:uppercase;padding:.65rem .6rem;color:#334155}
  .table tbody td{padding:.65rem .6rem;border-bottom:1px dashed #edf2f7}
  .table tbody tr:last-child td{border-bottom:0}

  /* Pills */
  .pill{padding:.2rem .55rem;border-radius:999px;font-size:.72rem;font-weight:800;border:1px solid var(--border)}
  ...
  /* Chips + bars */
  .chip{display:inline-flex;align-items:center;gap:.5rem;border-radius:14px;border:1px solid var(--border);padding:.4rem .6rem;background:rgba(255,255,255,.9)}
  ...
  .alert .badge{width:40px;height:40px;border-radius:12px;display:grid;place-items:center;border:1px solid var(--border);background:#fff}
...
@section('actions')
  <div class="toolbar">
    ...
    <form method="GET" class="ml-auto flex gap-2">
      <input class="input" type="text" name="q" value="{{ $q ?? '' }}" placeholder="Search products, batches, materials">
...
<div class="page-wrap space-y-6">
  ...
  <div class="card p-4">
    <div class="flex items-center justify-between">
      ...
      <div class="flex gap-2">
        <a href="{{ route('inventory.index') }}?export=csv" class="btn">Export CSV</a>
        <a href="{{ route('inventory.index') }}?export=pdf" class="btn">Export PDF</a>
      </div>
    </div>
  </div>
  ...
  <div class="card p-4">
    <div class="flex items-center justify-between mb-2">
      <h3 class="font-semibold">Notifications <span class="tag">{{ count($productionAlarms) }} active</span></h3>
  ...
  <div class="grid grid-cols-2 lg:grid-cols-6 gap-4">
    <div class="card p-4 kpi"><div class="flex items-center gap-3"><span class="icon">🥩</span><div><div class="meta">Products</div><div class="value">{{ $totalProducts }}</div></div></div></div>
    <div class="card p-4 kpi"><div class="flex items-center gap-3"><span class="icon">📦</span><div><div class="meta">Materials (kg)</div><div class="value">{{ number_format($totalMaterialsWeight,3) }}</div></div></div></div>
    <div class="card p-4 kpi"><div class="flex items-center gap-3"><span class="icon">🏷️</span><div><div class="meta">Batches</div><div class="value">{{ $batchesInProduction }}</div></div></div></div>
    <div class="card p-4 kpi"><div class="flex items-center gap-3"><span class="icon">✅</span><div><div class="meta">With Stock</div><div class="value">{{ $batchesReleased }}</div></div></div></div>
    <div class="card p-4 kpi"><div class="flex items-center gap-3"><span class="icon">⏳</span><div><div class="meta">Expiring ≤7d</div><div class="value" style="color:var(--warn)">{{ $batchesExpiringSoon }}</div></div></div></div>
    <div class="card p-4 kpi"><div class="flex items-center gap-3"><span class="icon">💸</span><div><div class="meta">Revenue (₱)</div><div class="value">{{ number_format($totalRevenue,2) }}</div></div></div></div>
  </div>
              ...
              <td>
                <div class="flex items-center gap-3">
                  ...
                  <div class="min-w-0">
                    ...
                    <div class="meta">Last prod: {{ optional($p->production_date)->format('Y-m-d') ?: '—' }}</div>
                  </div>
                </div>
              </td>
      ...
      <table class="table min-w-[720px]">
        ...
        <tbody>
          @forelse($materials as $m)
            @php $low = (float)$m->quantity_kg <= ($lowThresh ?? 5); @endphp
            <tr>
              <td>
      ...
      <div class="card p-4">
        ...
        <div class="table-wrap">
          <table class="table min-w-[520px]">
            <thead><tr><th>Material</th><th class="text-right">Qty Used</th><th class="text-right">Cost</th></tr></thead>
            <tbody>
        ...
        <ul class="space-y-2 max-h-[360px] overflow-auto pr-1">
          @forelse($expiringSoon as $b)
            @php
              ...
              $ratio = max(0,min(100, 100-($days ?? 0)*100/7));
            @endphp
            <li class="p-2 rounded-lg border border-[var(--border)] bg-white">
              <div class="flex items-center justify-between">
                <div class="min-w-0">
                  <div class="truncate font-medium">
                    {{ $b->product?->product_name }} <span class="meta">({{ $b->batch_number }})</span>
                  </div>
                  <div class="meta">Exp: {{ optional($b->expiration_date)->format('Y-m-d') ?? '—' }}</div>
                </div>
                <div class="text-right">
                  <span class="pill {{ $cls }}">{{ $days }}d</span>
                  <div class="meta mt-1">{{ number_format((float)($b->current_inventory ?? 0),3) }} kg</div>
                </div>
              </div>
              <div class="bar mt-2"><i style="background:{{ $days<=3?'var(--bad)':($days<=7?'var(--warn)':'var(--ok)') }};width:{{ $ratio }}%"></i></div>
            </li>
          @empty
