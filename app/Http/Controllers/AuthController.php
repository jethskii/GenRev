...
use Illuminate\Support\Facades\RateLimiter; // ✅ added for requestOtp()
...
class AuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        try {
            ...
            if ($this->isMissingTableError($e)) {
                return back()
                    ->withInput($request->only('email', 'remember'))
                    ->withErrors(['email' => 'A required table is missing (users or employees). Run your migrations and clear caches.',
                    ]);
            }
            throw $e;
        }
    ...
    public function register(Request $request)
    {
        ...
        try {
            ...
            if ($this->isMissingTableError($e)) {
                return back()
                    ->withInput($request->except('password', 'password_confirmation'))
                    ->withErrors(['email' => "A required table is missing. Ensure your auth and employees tables exist, then run:\n".
    ...
    {
        // 42S02 = MySQL "table doesn't exist"; 42P01 = Postgres "undefined table".
        // This app deploys on Postgres (Neon) in production, so the Postgres code/message
        // must be checked too, or every case below silently falls through to a raw 500.
        return in_array((string) $e->getCode(), ['42S02', '42P01'], true)
            || Str::contains($e->getMessage(), ['SQLSTATE[42S02]', 'SQLSTATE[42P01]']);
    }

    private function isUniqueConstraintError(QueryException $e): bool
    {
        // 23000 = MySQL's generic integrity-constraint code (covers duplicate keys);
        // 23505 = Postgres's specific unique-violation code. Same message caveat as above.
        return in_array((string) $e->getCode(), ['23000', '23505'], true)
            || Str::contains($e->getMessage(), ['Duplicate entry', 'duplicate key value violates unique constraint']);
    }
